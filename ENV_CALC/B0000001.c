...
vCAST_boolean vcast_proc_handles_command(int vc_m)
{
   ...
   for (vc_i=0; vCAST_PARAMETER[vc_i] != 0; vc_i++) {
                             ...
                             int *vCAST_LAST_VAL)
{
  ...
  if ((vc_grv_INDEX == -1) || (vCAST_PARAMETER[vc_grv_INDEX] == '*'))
  {
     *vCAST_FIRST_VAL = 0;
     *vCAST_LAST_VAL  = 0;
  }
...
void VCAST_TI_BITFIELD ( VCAST_LONGEST_INT *vc_VAL, int Bits,  vCAST_boolean is_signed )
{
   ...
   case vCAST_SET_VAL :
      if ( is_signed == vCAST_true )
         *vc_VAL = VCAST_PARAM_AS_LONGEST_INT();
      else
         *vc_VAL = VCAST_PARAM_AS_LONGEST_UNSIGNED();
      break;
...
void VCAST_TI_STRING (char **vcast_param, int from_bounded_array,int size_of_bounded_array ) {
   vcast_not_supported ();
}
#else
...
/**************************************************************************
Function: vcast_unconvert_string
Parameters: vcast_input - string to translate
            vcast_use_hex - non-zero to use hex (vs octal)
            vcast_length - length of string to convert (may not be strlen)
Description:  This function takes a string and returns it with all the 
characters translated into hexadecimal or octal numerals.  This translation
is used to prevent nongraphical characters from creating results that span
more than one line.  This also allows nongraphical characters to be compared
to expected results since some characters such as tabs and spaces would
otherwise be indistinguishable.
 *************************************************************************/
char * vcast_unconvert_string(char* vcast_input, int vcast_use_hex, int vcast_length)
{
  int index = 0;
  char* output = vcastCommonOutputBuffer;
  vcastCommonOutputBuffer[0] = '\0';
  while (index < vcast_length)
  {
    vcast_char_to_based_string(output, vcast_input[index], vcast_use_hex);
    output += VCAST_strlen(output);
    ++index;
  }
         ...
         if ((*vcast_param) == 0) 
            ...
            if (vCAST_FULL_STRINGS && from_bounded_array) {
               if (vcast_contains_unprintable_with_length(*vcast_param, size_of_bounded_array)){
                  vectorcast_fprint_string(vCAST_OUTPUT_FILE, vcast_unconvert_string(*vcast_param, vCAST_HEX_NOTATION, size_of_bounded_array));
                  vectorcast_fprint_string(vCAST_OUTPUT_FILE, "\n");
               } else {
               ...
               if (vcast_contains_unprintable(*vcast_param)){
                  vectorcast_fprint_string(vCAST_OUTPUT_FILE, vcast_unconvert_string(*vcast_param, vCAST_HEX_NOTATION, VCAST_strlen(*vcast_param)));
                  vectorcast_fprint_string(vCAST_OUTPUT_FILE, "\n");
               } else
...
void vCAST_RUN_DATA_IF (const char* VCAST_PARAM, vCAST_boolean POST_CONSTRUCTOR_USER_CODE)
{
  ...
  const char* vcast_param_value = vCAST_PARAMETER;
  ...
  vcast_param_value += (vCAST_INDEX + 1);
  if (VCAST_strcmp( vcast_param_value, "<<null>>" ) == 0)
     vCAST_VALUE_NUL = vCAST_true;
  vCAST_PARAMETER_KEY[0] = '\0';
  ...
  else if (VCAST_strncmp(vcast_param_value, "<<MIN>>", 7) == 0)
    vCAST_COMMAND = vCAST_FIRST_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<MID>>", 7) == 0)
    vCAST_COMMAND = vCAST_MID_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<MAX>>", 7) == 0)
    vCAST_COMMAND = vCAST_LAST_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<POS_INF>>", 11) == 0)
    vCAST_COMMAND = vCAST_POS_INF_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<NEG_INF>>", 11) == 0)
    vCAST_COMMAND = vCAST_NEG_INF_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<NAN>>", 7) == 0)
    vCAST_COMMAND = vCAST_NAN_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<MIN-1>>", 9) == 0)
    vCAST_COMMAND = vCAST_MIN_MINUS_1_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<MAX+1>>", 9) == 0)
    vCAST_COMMAND = vCAST_MAX_PLUS_1_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<ZERO>>", 8) == 0)
    vCAST_COMMAND = vCAST_ZERO_VAL;
  else if (VCAST_strncmp(vcast_param_value, "<<KEEP>>", 8) == 0)
    vCAST_COMMAND = vCAST_KEEP_VAL;
  else if (VCAST_strcmp(vcast_param_value, "<<STUB>>") == 0)
...
#endif
  vcast_B1_switch( UNIT_INDEX, SUB_INDEX, PARAM_INDEX, vcast_param_value );
  vCAST_PARAMETER_KEY[0] = '\0';
  vCAST_PARAMETER[0] = '\0';

  if ( vCAST_TOOL_ERROR )
    {
    if ( (vCAST_COMMAND != vCAST_PRINT) && (vCAST_COMMAND != vCAST_ALLOCATE) )
      {
...
VCAST_LONGEST_INT VCAST_PARAM_AS_LONGEST_INT()
{
  return VCAST_atoi( vCAST_PARAMETER + vCAST_INDEX + 1 );
}

VCAST_LONGEST_UNSIGNED VCAST_PARAM_AS_LONGEST_UNSIGNED()
{
  VCAST_LONGEST_UNSIGNED retVal = 0;
  /* Convert negative numbers like -3 to integer first, then cast to unsigned
   * to avoid just setting them to zero */
  if (vCAST_PARAMETER[vCAST_INDEX + 1] == '-')
  {
    VCAST_LONGEST_INT intermediate = VCAST_PARAM_AS_LONGEST_INT();
    retVal = (VCAST_LONGEST_UNSIGNED)(intermediate);
  }
  else
  {
    retVal = VCAST_strtoul ( vCAST_PARAMETER + vCAST_INDEX + 1, (char **)VCAST_NULL, 10 );
  }
  return retVal;
}

vCAST_long_double VCAST_PARAM_AS_LONGEST_FLOAT()
{
  vCAST_long_double retVal = 0.0;
  vCAST_STR_TO_LONG_DOUBLE(vCAST_PARAMETER + vCAST_INDEX + 1, &retVal);
  return retVal;
}
